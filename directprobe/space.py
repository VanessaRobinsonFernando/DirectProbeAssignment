...
class Space:
    ...
    def overlapping(self, q: DistanceQ, t: Cluster) -> bool:
        """ Check if the new cluster overlaps with other clusters
    ...
    def _sphere_overlapping(self, q: DistanceQ, newcluster: Cluster) -> Tensor:
        ...
        # 3. It is an active cluster
        mask = overlap_mask & indexs & q.active
        return torch.nonzero(mask).reshape(-1)

    def _lp_overlapping(self, q: DistanceQ, newcluster: Cluster, indexs: List[int]) -> bool:
        ...
        """
        # Extract data for the new cluster
        cur_vecs = q.fix_embeddings[torch.LongTensor(newcluster.indices)]
        X1 = cur_vecs.numpy()

        # Prepare the data for parallel processing
        tasks = [(X1, q.fix_embeddings[torch.LongTensor(q.clusters[i].indices)].numpy()) for i in indexs]

        # Function to process each task
        def task(data):
            X1, X2 = data
            return self.solver(X1, X2)
        
        # Perform parallel processing
        results = Parallel(n_jobs=30, prefer='processes', verbose=0)(delayed(task)(data) for data in tasks)

        # Update the white and black lists based on the results
